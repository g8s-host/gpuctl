...
def test_add_node_label(mock_get_instance, client):
    """测试为节点添加标签API"""
...
@patch('server.routes.labels.PoolClient.get_instance')
def test_remove_node_label(mock_get_instance, client):
    """测试删除节点标签API"""
    mock_instance = MagicMock()
...
@patch('server.routes.labels.PoolClient.get_instance')
def test_update_node_label(mock_get_instance, client):
    """测试更新节点标签API"""
    mock_instance = MagicMock()
    mock_instance._label_node = MagicMock(return_value=True)
    mock_instance.get_node = MagicMock(return_value={"name": "node-1"})
...
@patch('server.routes.labels.PoolClient.get_instance')
def test_list_all_node_labels(mock_get_instance, client):
    """测试获取所有节点标签API"""
    mock_instance = MagicMock()
