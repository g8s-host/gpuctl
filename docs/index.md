...
<table width="100%">
  ...
  <tr>
    ...
    <td><b>Pre-installed images, rendezvous env vars injected automatically</b><br>Use official images with DeepSpeed, VLLM, LlamaFactory pre-installed. For <code>mode: multi-node</code> jobs the platform injects DDP rendezvous env vars (MASTER_ADDR, MASTER_PORT, WORLD_SIZE, RANK, LOCAL_RANK) and creates the Headless Service — your launcher (e.g. torchrun) consumes them</td>
  </tr>
  <tr>
  ...
  <tr>
    <td><b>😵 Complex Multi-Node Distributed Setup</b><br>Indexed Job, Headless Service, and rendezvous wiring are verbose</td>
    <td>Multi-node training requires hand-writing an Indexed/JobSet, a Headless Service for peer discovery, wiring MASTER_ADDR/RANK/WORLD_SIZE, and provisioning shared storage. Misconfiguration leads to training hangs</td>
    <td><b>Declare workers, distributed plumbing created automatically</b><br>Set resources.gpu and <code>distributed: {mode: multi-node, workers: N}</code>. The platform creates the Indexed Job + Headless Service, injects DDP rendezvous env vars, and gives every worker the same NFS /home/jovyan for checkpoints</td>
  </tr>
  <tr>
