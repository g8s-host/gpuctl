...
<table width="100%">
  ...
  <tr>
    ...
    <td><b>镜像中预装，自动注入通信环境变量</b><br>提供预装 DeepSpeed、VLLM、LlamaFactory 的官方镜像。对 <code>mode: multi-node</code> 任务，平台自动注入 DDP 通信环境变量（MASTER_ADDR、MASTER_PORT、WORLD_SIZE、RANK、LOCAL_RANK）并创建 Headless Service —— 由你的启动器（如 torchrun）消费</td>
  </tr>
  <tr>
  ...
  <tr>
    <td><b>😵 多机分布式配置复杂</b><br>Indexed Job、Headless Service、通信参数繁琐</td>
    <td>多机训练需要手写 Indexed/JobSet、用于互相发现的 Headless Service，串联 MASTER_ADDR/RANK/WORLD_SIZE，还要准备共享存储。配置错误导致训练卡死，调试困难</td>
    <td><b>声明 workers 数量，分布式管道自动搭建</b><br>设置 resources.gpu 和 <code>distributed: {mode: multi-node, workers: N}</code>，平台自动创建 Indexed Job + Headless Service，注入 DDP 通信环境变量，并让每个 Worker 共享同一块 NFS /home/jovyan 写 checkpoint</td>
  </tr>
  <tr>
