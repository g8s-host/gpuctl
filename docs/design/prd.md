...
3.  **显存与稳定性保障**：平台实时监控单卡显存占用（默认阈值≤75GB），当某节点显存使用率突增时，自动触发梯度检查点强化策略并降低该节点临时批量大小；支持断点续训，通过`output.resume_from_checkpoint: true`配置，训练中断后可通过`gpuctl resume job `恢复训练，避免数据丢失。实测单卡显存占用稳定在68-72GB，跨节点通信延迟≤2ms，训练吞吐量较手动配置Deepspeed提升18%。
...
**平台适配验证**：完全满足。工程师在YAML中指定VLLM官方镜像（`vllm/vllm-serving:latest`），通过`environment.command`配置VLLM启动命令（`["python", "-m", "vllm.entrypoints.openai.api_server", "--model", "llama3-8b"]`），并通过`autoscaling`字段设置目标GPU利用率70%，平台将自动转换为K8s Deployment+Service+HPA资源，实现基于负载的弹性扩缩容，充分发挥VLLM的高并发优势。
...
| `gpuctl get nodes --gpu-type runwhere.ai/a100-80g` | 过滤查询带有指定 GPU 类型的所有节点                          | `kubectl get nodes -l runwhere.ai/gpu-type=a100-80g` + 节点状态校验 |
...
| `gpuctl get nodes --gpu-type runwhere.ai/gpu-type` | 仅查询所有节点的 GPU 类型 Label（键值对形式）                | `kubectl get nodes -o jsonpath='{range .items[*]}{.metadata.name}{"\t"}{.metadata.labels.nvidia\.com/gpu-type}{"\n"}{end}'` |

节点管理类 gpuctl CLI 命令（按指定格式）：label命令的key必须要以"runwhere.ai/"开头，以避免和其他人员label的key冲突。十分重要，注意！！！

| 命令示例                                                     | 功能描述                                                     | 底层近似操作                                                 |
| ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| `gpuctl label node node-1 runwhere.ai/gpu-type=a100-80g`        | 给指定节点标记 GPU 类型 Label（默认 Label 键）               | `kubectl label nodes node-1 runwhere.ai/gpu-type=a100-80g`    |
| `gpuctl label node node-2 node-3 runwhere.ai/gpu-type=a100-40g --overwrite` | 批量给多个节点标记自定义 GPU 类型 Label，支持覆盖已有同键 Label | `for node in node-2 node-3; do kubectl label nodes $node company.com/gpu-model=a100-40g --overwrite; done` |
| `gpuctl get label node-1 --key=runwhere.ai/gpu-type`            | 查询指定节点的指定 GPU 类型 Label 值                         | `kubectl get node node-1 -o jsonpath='{.metadata.labels.nvidia\.com/gpu-type}'` |
...
| `gpuctl label node node-1 runwhere.ai/gpu-type --delete`        | 删除指定节点的指定 GPU 类型 Label                            | `kubectl label nodes node-1 runwhere.ai/gpu-type-`（K8s 删除 Label 的标准语法） |
...
- 响应（200 OK）：json
  ...
  ```json
  {
    ...
    "labels": [
      {"key": "runwhere.ai/gpu-type", "value": "a100-80g"},
...
#### （对应 `gpuctl get nodes --label runwhere.ai/gpu-type`）
...
- 查询参数：
  ...
  - `key`：必填，指定要查询的 Label 键（如 `runwhere.ai/gpu-type`）
  - `page`：分页页码，默认 1
  - `pageSize`：每页数量，默认 20
...
- 响应（200 OK）：json
  ...
  ```json
  {
    "total": 5,
    "items": [
      {"nodeName": "node-1", "labelKey": "runwhere.ai/gpu-type", "labelValue": "a100-80g"},
      {"nodeName": "node-2", "labelKey": "runwhere.ai/gpu-type", "labelValue": "a100-80g"},
      {"nodeName": "node-3", "labelKey": "runwhere.ai/gpu-type", "labelValue": "a100-40g"},
      {"nodeName": "node-4", "labelKey": "runwhere.ai/gpu-type", "labelValue": "t4"},
      {"nodeName": "node-5", "labelKey": "runwhere.ai/gpu-type", "labelValue": "t4"}
    ]
  }
  ```
...
#### （对应 `gpuctl label node node-1 runwhere.ai/gpu-type=a100-80g`）
...
- 请求体：json
  ...
  ```json
  {
    "key": "runwhere.ai/gpu-type", // Label 键（支持自定义）
...
- 错误响应（409 Conflict）：json
  ...
  {
    "error": "LabelConflict",
    "message": "节点 node-1 已存在键为 runwhere.ai/gpu-type 的 Label，如需覆盖请设置 overwrite=true"
  }
  ```
...
- 请求体：json
  ...
  ```json
  {
    ...
    "key": "runwhere.ai/gpu-type",    // Label 键
...
#### （对应 `gpuctl get node-labels node-1 --key=runwhere.ai/gpu-type`）
...
- 路径参数：
  ...
  - `nodeName`：节点名称（如 `node-1`）
  - `key`：Label 键（如 `runwhere.ai/gpu-type`）
...
- 错误响应（404 Not Found）：json
  ...
  {
    "error": "LabelNotFound",
    "message": "节点 node-1 未找到键为 runwhere.ai/gpu-type 的 Label"
  }
  ```
  ...
  ```json
  {
    ...
    "items": [
      ...
      },
      {
        "nodeName": "node-4",
        "gpuLabels": [
          {"key": "runwhere.ai/gpu-type", "value": "t4"}
        ],
...
#### （对应 `gpuctl label node node-1 runwhere.ai/gpu-type --delete`）
