...
| `runwhere.ai/namespace` | namespace 名称 | 记录所属命名空间，便于跨 namespace 快速定位 |
...
| **Priority** | 优先级 | label `runwhere.ai/priority` |
| **Pool** | 资源池 | label `runwhere.ai/pool` |
...
| **GPU TYPE** | GPU 型号 | label `runwhere.ai/gpuType` |
| **IP** | 节点 IP | `node.status.addresses[InternalIP]` |
| **POOL** | 所属资源池 | label `runwhere.ai/pool` |
...
> 仅显示 gpuctl 创建的命名空间（label `runwhere.ai/namespace=true`）+ default。
...
Pod
  ...
  pod.labels.runwhere.ai/job-type = "inference"
  pod.labels.runwhere.ai/namespace = "default"             ← 记录所属 namespace
      │
      ▼
get jobs 输出
