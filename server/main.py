...
# 导入我们之前实现的模块
from gpuctl.parser import BaseParser, ParserError
...
class LogResponse(BaseModel):
    logs: List[str]
    lastTimestamp: Optional[datetime]
...
@app.post("/api/v1/jobs/batch", response_model=BatchCreateResponse, status_code=201)
...
@app.get("/api/v1/jobs/{jobId}", response_model=Dict[str, Any])
...
@app.post("/api/v1/jobs/{jobId}/pause")
...
@app.post("/api/v1/jobs/{jobId}/resume")
...
# 7.2.2 资源池管理API
@app.get("/api/v1/pools", response_model=List[PoolResponse])
...
@app.get("/api/v1/pools/{poolName}", response_model=Dict[str, Any])
...
# 7.2.3 监控与日志API
@app.get("/api/v1/jobs/{jobId}/logs", response_model=LogResponse)
...
@app.websocket("/api/v1/jobs/{jobId}/logs/ws")
...
@app.get("/api/v1/jobs/{jobId}/metrics", response_model=Dict[str, Any])
...
# 7.2.4 权限管理API
@app.post("/api/v1/auth/check", response_model=AuthCheckResponse)
